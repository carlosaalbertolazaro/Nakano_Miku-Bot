...
// Antes esto marcaba cada respuesta del bot como "reenviada muchas veces" y
// la asociaba a un canal de WhatsApp de terceros (contextInfo.forwardedNewsletterMessageInfo
// con un newsletterJid hardcodeado del autor original), lo que hacía que
// WhatsApp mostrara el aviso de spam/reenvío y un botón "Canal" enlazando a
// ese canal ajeno en TODOS los mensajes del bot. Se saca por completo.
...
export function serializarM(conn, m) {
  ...
  if (m.msg?.contextInfo?.quotedMessage) {
    ...
    // Comparar solo contra conn.user.id no alcanza: WhatsApp a veces manda el
    // participant citado como @lid en vez del @s.whatsapp.net de siempre (el
    // mismo fenómeno ya documentado en handler.js/getAdminStatus para admins).
    // Sin este fallback, responder a un mensaje real de Miku podía marcar
    // fromMe:false por un simple desfase de formato de JID, rompiendo en
    // silencio cualquier plugin que dependa de "esto es una respuesta al bot"
    // (ia-chat.js entre otros).
    const botIdsQuoted = [conn.user?.id, conn.user?.lid].filter(Boolean).map(id => jidNormalizedUser(id))
    m.quoted.fromMe = botIdsQuoted.includes(m.quoted.sender)

...
async function attemptOnce(apiKey, contents, timeoutMs) {
  ...
  if (!res.ok) {
    const err = new Error(json?.error?.message || `Gemini respondió con estado ${res.status}`)
...
export async function askGemini(prompt, history = []) {
  ...
  // Gemini a veces tarda o devuelve 503 "high demand" de forma intermitente
  // (visto en vivo probando la key real de Carlos) — un solo reintento con
  // más margen de tiempo suele alcanzar sin hacer esperar demasiado al chat.
  try {
    return await attemptOnce(apiKey, contents, 25000)
  } catch (e) {
    const esTimeout = e.name === 'AbortError'
    const esSobrecarga = e.status === 503
    if (!esTimeout && !esSobrecarga) throw e

    try {
      return await attemptOnce(apiKey, contents, 35000)
    } catch (e2) {
      if (e2.name === 'AbortError') {
        throw new Error('Gemini tardó demasiado en responder (puede estar saturado). Probá de nuevo en un momento.')
      }
      throw e2
    }
  }
